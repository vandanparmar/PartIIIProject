...
# ylabel_arr = ['L-Aspartate Uptake','Citrate Uptake','Acetate Transport','Pentose Phosphate Transport','Galactose Synthesis','Pentose Phosphate Transport','Ubiquinone Synthesis','Water Transport','Acid Transport','Galactokinase Synthesis','Acid Equilibriation']
# xlabel_arr = ['Oxygen Uptake','Oxygen Uptake','Oxygen Uptake','Arginine Uptake','Arginine Uptake','Arginine Uptake','Oxygen Uptake','Oxygen Uptake','Oxygen Uptake','Arginine Uptake','Oxygen Uptake']
obj2_arr = []
...
filename_arr = ['hp_'+name.split('_')[1] for name in obj2_arr]
ylabel_arr = ['Intake']*len(obj1_arr)
xlabel_arr = ['Biomass Production']*len(obj1_arr)

model_arr = ['h_pylori.json']*len(obj1_arr)


data_str = 'saturated_data/data'
...
for obj1_str,obj2_str,model_str,filename,xlabel,ylabel in tqdm(list(zip(obj1_arr, obj2_arr, model_arr,filename_arr,xlabel_arr,ylabel_arr))):
	...
	for i,reaction in enumerate(model.reactions):
		if 'EX_' in reaction.id:
			...
			reaction.bounds = np.clip(reaction.bounds, -100.0, 100.0)
